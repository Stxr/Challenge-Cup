...
void pageDisplay(WM_MESSAGE *pMsg) {
	...
	{
	case WM_TIMER:
		BEEP = getBeep(id_bottle); //蜂鸣器随着警报器一起报警
		if(getBeep(id_bottle)){
			testSpeed=0;
		}
		if (dataDisplay[id_bottle][0] == 0 || (dataDisplay[id_bottle][3] <= 0 && dataDisplay[id_bottle][4] <= 0)) { //dataDisplay[][3]和dataDisplay[][4]有可能小于0
			setMotorMax(id_bottle); //换瓶了，把已经输完的夹紧
					flag_speed=0;
			id_bottle++;
			if (id_bottle >= 4) id_bottle = 0;
		}
		maskOtherExti(id_bottle); //其他中断屏蔽
		if (dataDisplay[id_bottle][3] > 0 || dataDisplay[id_bottle][4] > 0) { //剩余输液时间是否为0
			(*Time_Used)++;
			if (*Time_Used % 2) {
			...
			if (dataDisplay[id_bottle][3] != 0) { //速率1
				if(dataDisplay[id_bottle][4]==0){  //如果没有速率2了，自动变蓝
				...
				if(*Time_Left ==0){
					...
					BEEP_ms(500);			//切换的时候长响
//					stepMotor_Distance(id_bottle,5,  -(*speed)*10); //speed越大，distance越小，流量越大。
					stepMotor_Distance(id_bottle,2,  -120);//往后退30，等待调节
				...
				if(*Time_Left == 0){
					...
					stepMotor_Distance(id_bottle,2,(*speed-dataDisplay[id_bottle][2])*5); //切换速率2
			...
			if(*Time_Used %2 ==0){
				if(  *speed >	testSpeed + 1 || *speed < testSpeed  - 1 ){  //速度容差
						stepMotor_Distance(id_bottle,2, 1+(int)((testSpeed - (*speed))*3)); //如果比实际大，则挤压，一次前进0.01*3mm	
			...
			if(id_bottle == last_bottle){  //如果是最后一瓶了
				if(dataDisplay[id_bottle][3] ==0 || dataDisplay[id_bottle][4]==0){ //最后的速率
			...
			if (dataDisplay[id_bottle][4] <= 0 && dataDisplay[id_bottle][3] <= 0) { //输液完成
...
//		}
	...
	case WM_CREATE:
		...
		*Time_Used =0;
		...
		for (i = 0; i < 4; i++) {
			if (dataDisplay[i][0] == 0)continue;//如果容量为0，则不创建
	...
	case WM_PID_STATE_CHANGED:
		...
		if ((pState->StatePrev == 0) && (pState->State == 1)) {
			if (pState->y >= 222) {
				...
				if (pState->x >= 112 && pState->x <= 149) { //左边，减小speed
				...
				else if (pState->x >= 269 && pState->x <= 307) {//右边，增大speed
					stepMotor_Distance(id_bottle,2,8);
					if (*speed >= 10) {
						*speed = 10;
						break;
...
//				test11 =test11;
					...
					stepMotor_Write(RESETBIT,0); //清除初始化
